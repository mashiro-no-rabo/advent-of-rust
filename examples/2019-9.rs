...
fn read_param(param: i64, mode: u8, mem: &[i64], rbase: usize) -> i64 {
  match mode {
  ...
  loop {
    ...
    match inst {
      ...
      1 => {
        // plus
        let pstart = pc + 1;
        let op1 = read_param(mem[pstart], pmode[0], &mem, relative_base);
        let op2 = read_param(mem[pstart + 1], pmode[1], &mem, relative_base);
      ...
      4 => {
        // output
        let output = read_param(mem[pc + 1], pmode[0], &mem, relative_base);
        outputs.push(output);
        pc += 2;
      }
      5 => {
        // jump-if-true
        let op1 = read_param(mem[pc + 1], pmode[0], &mem, relative_base);
        if op1 != 0 {
          pc = read_param(mem[pc + 2], pmode[1], &mem, relative_base) as usize
        } else {
          pc += 3;
        }
      }
      ...
      9 => {
        ...
        let offset = read_param(mem[pc + 1], pmode[0], &mem, relative_base);
